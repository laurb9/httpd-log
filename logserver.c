...
#undef WRITE_LOG_ADD
    ...
    if (child_counter > MAX_LIVE_CHILDREN){
      LOG_PRINTF( DEBUG_MIN, ZONE, "wait_loop: stopped receiving data, too many children (%d)", child_counter );
      nfds = 0;
    } else {
...
void process_batch( void ){
  ...
  if (detach){
    ...
    pid = -1;
    i = -1;
    LOG_PRINTF( DEBUG_MIN, ZONE, "fg mode, staying in one piece (no fork)", debug );
  }
  if (pid < 0 && i == 0){
    log_printf( DEBUG_ERROR, ZONE,
  ...
  if (pid <= 0){
     ...
     */
    if (i == 0){
      LOG_PRINTF( DEBUG_MAX, ZONE, "process_batch: forked child(PID=%d)", pid );
      child_counter++;
    }
  }
  log_counter = 0;
